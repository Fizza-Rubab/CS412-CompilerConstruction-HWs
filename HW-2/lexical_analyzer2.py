...
'The Lexer class is same as implemented in Homework 1.'
...
class Lexer:
    def __init__(self):
        ...
        '''
        self.keywords = ["and", "break", "continue", "else", "false", "for", "if", "mod", "not", "or", "then", "true", "while", "return"]
        self.punctuators = ["{", "}", "(", ")", ";", "[", "]", ",", "."]
        self.id_punctuators = self.punctuators.copy()
        self.id_punctuators.extend(["'", '"'])
        self.data_types = ["bool", "char", "int", "float", "void", "double", "str"]
        self.rel_operators = [">", "<", ">=", "<=", "==", "!="]
        ...
        self.line_no = 1
        self.token_stream=""
        self.token_stream_line = []
        self.errors_list = []
        self.id_num=1
        self.errors = "<line_no> <error_found>\n"
        self.end = False
        self.peek=""
        self.test_file = ""
        self.test_file_name = ""
        ...
        if self.index+1<self.length and self.text_stream[self.index+1]=='$':
            ...
            while not self.end:
                if temp_index>=self.length:
                    ...
                    self.errors_list.append(str(self.line_no)+" (incomplete comment)\n")
            ...
            while not self.end:
                if temp_index>=self.length:
                    ...
                    if len(halfword)<=1:
                        ...
                        self.errors_list.append('%s "%s (Invalid char constant)\n'%(self.line_no, halfword))
                    else:
                        self.errors+='%s "%s (Invalid string literal)\n'%(self.line_no, halfword)
                        self.errors_list.append('%s "%s (Invalid string literal)\n'%(self.line_no, halfword))
                    self.end = True
                elif self.text_stream[temp_index]=='"':
                    word = "".join(self.buffer)
                    if len(word)==0:
                        self.token_stream+="<character,>"%word
                        self.token_stream_line.append([self.line_no, "<character,>"%word] )
                    if len(word)==1:
                        self.token_stream+="<character,%s>"%word
                        self.token_stream_line.append([self.line_no, "<character,%s>"%word] )
                    else:
                        self.token_stream+="<literal,%s>"%word
                        self.token_stream_line.append([self.line_no, "<literal,%s>"%word] )
                    temp_index+=1
                    break
                elif self.text_stream[temp_index]=='\n':
                    halfword = "".join(self.buffer)
    ...
    def singlequoteAnalyze(self): 
        ...
        if self.index+1<self.length:
            ...
            self.errors_list.append("%s ' (Invalid char constant)\n"%self.line_no)
    #-------------------------------------
    #-------------------------------------
    def digitAnalyze(self):
        ...
        if self.is_number(num):
            ...
            self.token_stream_line.append([self.line_no, ("<num,%s>"%num)])
        else:
            self.errors+="%s (%s invalid number)\n"%(self.line_no, num)
            self.errors_list.append("%s (%s invalid number)\n"%(self.line_no, num))
    #-------------------------------------
    #-------------------------------------
    def alphaAnalyze(self):
        ...
        if self.is_keyword(word):
            ...
            self.token_stream_line.append([self.line_no, ("<%s>"%word)])
        elif self.is_datatype(word):
            self.token_stream+=("<dt,%s>"%word)
            self.token_stream_line.append([self.line_no, ("<dt,%s>"%word)])
            ...
            if check==False:
                ...
                self.token_stream_line.append([self.line_no, ("<id,%s>"%self.id_num)])
                self.id_num+=1
            else:
                self.token_stream+=("<id,%d>"%check)
                self.token_stream_line.append([self.line_no, ("<id,%d>"%check)])
        else:
            self.errors+=(str(self.line_no)+"%s (unrecognized token)\n")%word
            self.errors_list.append((str(self.line_no)+"%s (unrecognized token)\n")%word)
    #-------------------------------------
    #-------------------------------------
    def notAnalyze(self):
        '''
        ...
        '''
        if self.index+1<self.length and self.text_stream[self.index+1]=='=':
            self.token_stream+="<rel_op,NOTEQ>"
            self.token_stream_line.append([self.line_no, "<rel_op,NOTEQ>"])
            self.index+=1
        else:
            self.errors+="%s ! (unrecognized symbol)\n"%(self.line_no)
            self.errors_list.append("%s ! (unrecognized symbol)\n"%(self.line_no))
    #-------------------------------------
    #-------------------------------------
    def greatAnalyze(self):
        ...
        '''
        if self.index+1<self.length and self.text_stream[self.index+1]=='=':
            self.token_stream+="<rel_op,GTEQ>"
            self.token_stream_line.append([self.line_no, "<rel_op,GTEQ>"])
            self.index+=1
        else:
            self.token_stream+="<rel_op,GT>"
            self.token_stream_line.append([self.line_no, "<rel_op,GT>"])
    ...
    def lessAnalyze(self):
        ...
        '''
        if self.index+1<self.length and self.text_stream[self.index+1]=='=':
            self.token_stream+="<rel_op,LTEQ>"
            self.token_stream_line.append([self.line_no, "<rel_op,LTEQ>"])
            self.index+=1
        else:
            self.token_stream+="<rel_op,LT>"
            self.token_stream_line.append([self.line_no, "<rel_op,LT>"])
    #-------------------------------------
    #-------------------------------------
    def eqAnalyze(self):
        '''
        ...
        '''
        if self.index+1<self.length and self.text_stream[self.index+1]=='=':
            self.token_stream+="<rel_op,EQEQ>"
            self.token_stream_line.append([self.line_no, "<rel_op,EQEQ>"])
            self.index+=1
        else:
            self.token_stream+="<=>"
            self.token_stream_line.append([self.line_no, "<=>"])
    #-------------------------------------
    #-------------------------------------
    def analyze(self):
        '''
        ...
        while(not self.end):
            ...
            if self.peek=='+':
                ...
                self.token_stream_line.append([self.line_no, "<%s>"%self.peek])
            elif self.peek=='/':
                self.commentAnalyze()   
            elif self.peek=='>':
                self.greatAnalyze()
                ...
                self.errors_list.append("%s %s (unrecognized symbol)\n"%(self.line_no, self.peek))
            elif self.peek in self.punctuators:
                self.token_stream+="<%s>"%self.peek
                self.token_stream_line.append([self.line_no, "<%s>"%self.peek])
    ...
    def output(self):
        ...
        symbol_file.close()
        return self.token_stream_line, self.errors
    #-------------------------------------
    #-------------------------------------        
